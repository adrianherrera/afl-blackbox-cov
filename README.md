...
An external watchdog (`afl-blackbox-watchdog.py`) watches the directory where
these testcases are written to removes any testcases that do not produce new
coverage. The steps to use the watchdog are as follows:

1. Compile both instrumented and uninstrumented binaries.
2. Start the blackbox fuzzing campaign with `afl-blackbox-cov -n`.
3. Once `fuzzer_stats` has been created, run `afl-blackbox-watchdog.py` and use
   the same output directory as used in 2. Optionally, a CSV file containing
   coverage information can be generated by the watchdog via the `--csv`
   option.
