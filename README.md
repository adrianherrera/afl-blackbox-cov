...
An external monitor (`blackox-cov-scripts/afl-blackbox-monitor.py`) watches
the directory where these testcases are written to removes any testcases that
do not produce new coverage. The steps to use the monitor are as follows:
...
3. Once `fuzzer_stats` has been created, run
   `blackox-cov/afl-blackbox-monitor.py` and use the same output directory as
   used in 2. Optionally, a CSV file containing coverage information can be
   generated by the monitor via the `--csv` option.
